    ...
    public function changeCustomerStatus(Request $request)
    {
        ...
        try {
            if($request->type=='appointment'){
                $appoinetment=BookAppointment::findOrFail($request->id);
                $appoinetment->status='read';
                $appoinetment->save();
            }
            if($request->type=='query'){
                $appoinetment=CustomerQuery::findOrFail($request->id);
                $appoinetment->status=false;
                $appoinetment->save();
            }
            DB::commit();
            ...
            $exception->controller_function = "DashboardController.changeCustomerStatus";
            $exception->error = $e;
